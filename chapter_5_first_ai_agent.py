...
print(
    summary
)  # Expected output: Meeting request for next Wednesday at 3 PM. Awaiting response.
...
def suggest_reply(email_content):
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Suggest a professional reply to this email {email_content}",
            },
        ],
    )
    return response["choices"][0]["message"]["content"]
...
print(
    reply
)  # Expected output: Hello, thanks for reaching out! Next Wednesday at 3 PM works for me. Looking forward to it.
